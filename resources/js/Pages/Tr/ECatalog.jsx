            ...
            <WebSiteTRLayout address={address?.general_value} phone={phone?.general_value} email={email?.general_value} telegram={telegram?.general_value} linkdin={linkdin?.general_value} insta={insta?.general_value} about_tr={about_us_text_tr?.general_value}>
                <div className="bg-black py-16 bg-auto bg-no-repeat text-center flex flex-col lg:mt-0 mt-28">
                    <h5 ref={headTitle} className="text-3xl font-bold lg:text-6xl space-x-5">
                        <span className="inline-block first-letter:text-5xl lg:first-letter:text-7xl text-white">E-Katalog</span>
                    </h5>
                </div>
                    ...
                    <div className=" w-full flex flex-col lg:px-20 space-y-8 lg:space-y-4 justify-between text-center">
                        {
                            catalog_list.data.map((item, index) => (
                                <div key={index} className="lg:text-xl border flex flex-col lg:flex-row lg:justify-between lg:px-20 lg:items-center rounded-md py-8 shadow-lg" >
                                    ...
                                    <a href={`/storage/${item.download_filename}`} target="_blank" download={item.download_filename} className=" hover:text-green-800 h-full my-auto pt-5 lg:pt-0 transition-all duration-300  text-green-500" > İndirmek </a>
                    ...
                    {lastPage > 1 && (
                        <div className="flex flex-col items-center w-full py-3">
                            <div className="flex flex-row w-full h-32 items-center justify-center ">
                                <Link href="/tr/e-catalog?page=1" className="hover:text-green-800 transition-all duration-300  text-green-500">
                                    FitrstPage
                                </Link>
                                {range.map((page) => (
                                    <Link
                                        href={`/tr/e-catalog?page=${page}`}
                                ...
                                <Link href={`/tr/e-catalog?page=${lastPage}`} className="hover:text-green-800 transition-all duration-300 text-green-500">
                                    lastPage
                                </Link>
                            </div>
                        </div>
