...
const EditHero = ({ auth, hero_section_text_en , hero_section_text_tr }) => {


    const handleSubmit = (e) => {
        e.preventDefault();
        const formData = new FormData();
        formData.append('hero_section_text_en', hero_en_text.value);
        formData.append('hero_section_text_tr', hero_tr_text.value);
        Inertia.post('/edit-hero', formData);
    }
                    ...
                    <form onSubmit={(e) => handleSubmit(e)} className="justify-between items-center p-10   bg-white shadow-sm sm:rounded-lg">
                        <div className="flex flex-col lg:flex-row items-center justify-between gap-10 w-full border-4  border-green-400 rounded-md rounded-b-none text-center p-10 overflow-scroll lg:overflow-hidden">
                            ...
                            <div className="flex flex-col w-full">
                                ...
                                <textarea defaultValue={hero_section_text_tr.general_value} className="z-5 w-full h-48 border-t-4 rounded-lg hover:border-green-400 border-black-400 hover:rounded-xl duration-200" id="hero_tr_text"></textarea>
                            </div>
                            <div className="flex flex-col w-full">
                                <label className="text-2xl mb-4 hover:animate-pulse z-10" htmlFor="slogan_en">Slogan EN</label>
                                <textarea defaultValue={hero_section_text_en.general_value} className="z-5 w-full h-48 border-t-4 rounded-lg hover:border-green-400 border-black-400 hover:rounded-xl duration-200" id="hero_en_text"></textarea>
                            </div>
