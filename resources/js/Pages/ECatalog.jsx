                            ...
                            catalog_list.data.map((item, index) => (
                                <div key={index} className="lg:text-xl border flex flex-col lg:flex-row lg:justify-between lg:px-20 lg:items-center rounded-md py-8 shadow-lg" >
                                    <p className=" border-l-4 flex flex-row justify-between lg:justify-normal px-4 border-green-400" >
                                        Catalog Title: <span className="text-gray-500 lg:pl-10">{item.title}</span>
                                    </p>
                                    <p className=" border-l-4 flex flex-col lg:flex-row lg:items-center pt-4 lg:pt-0 justify-between lg:justify-normal px-4 border-green-400" >
                                        Catalog description: <span className="pt-2 lg:pt-0 text-gray-500 lg:pl-10">{item.description}</span>
                                    </p>
                                    <a target="_blank" href={`../../catalog/${item.download_filename}`} download={item.download_filename} className=" hover:text-green-800 h-full my-auto pt-5 lg:pt-0 transition-all duration-300  text-green-500" > Download </a>
                                </div>
                            )
                            )
                        }
                    ...
                    {lastPage > 1 && (
                        <div className="flex flex-col items-center w-full py-3">
                            <div className="flex flex-row w-full h-32 items-center justify-center ">
                                <Link href="/e-catalog?page=1" className="mx-1 hover:text-green-800 transition-all duration-300  text-green-500">
                                    FitrstPage
                                </Link>
                                {range.map((page) => (
                                    <Link
                                ...
                                <Link href={`/e-catalog?page=${lastPage}`} className="mx-1 hover:text-green-800 transition-all duration-300 text-green-500">
                                    lastPage
                                </Link>
                            </div>
                        </div>
